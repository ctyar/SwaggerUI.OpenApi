...
public partial class MyWebApplicationFactory<TEntryPoint> : IDisposable, IAsyncDisposable where TEntryPoint : class
{
    ...
    private readonly List<MyWebApplicationFactory<TEntryPoint>> _derivedFactories = new();

    /// <summary>
    /// <para>
    /// Creates an instance of <see cref="MyWebApplicationFactory{TEntryPoint}"/>. This factory can be used to
    ...
    /// The <see cref="MyWebApplicationFactory{TEntryPoint}"/> will find the entry point class of <typeparamref name="TEntryPoint"/>
    ...
    /// In case an attribute with the right key can't be found, <see cref="MyWebApplicationFactory{TEntryPoint}"/>
    ...
    /// <summary>
    /// Finalizes an instance of the <see cref="MyWebApplicationFactory{TEntryPoint}"/> class.
    /// </summary>
    ...
    /// <summary>
    /// Gets the <see cref="TestServer"/> created by this <see cref="MyWebApplicationFactory{TEntryPoint}"/>.
    /// </summary>
    public TestServer Server
    {
        get
        {
            if (_useKestrel)
            {
                throw new NotSupportedException();//TODO: Resources.TestServerNotSupportedWhenUsingKestrel);
            }
    ...
    /// <summary>
    /// Gets the <see cref="IServiceProvider"/> created by the server associated with this <see cref="MyWebApplicationFactory{TEntryPoint}"/>.
    /// </summary>
    ...
    /// <see cref="MyWebApplicationFactory{TEntryPoint}.WithWebHostBuilder(Action{IWebHostBuilder})"/>.
    /// </summary>
    public IReadOnlyList<MyWebApplicationFactory<TEntryPoint>> Factories => _derivedFactories.AsReadOnly();
    ...
    /// <summary>
    /// Creates a new <see cref="MyWebApplicationFactory{TEntryPoint}"/> with a <see cref="IWebHostBuilder"/>
    ...
    /// </param>
    /// <returns>A new <see cref="MyWebApplicationFactory{TEntryPoint}"/>.</returns>
    public MyWebApplicationFactory<TEntryPoint> WithWebHostBuilder(Action<IWebHostBuilder> configuration) =>
        WithWebHostBuilderCore(configuration);

    internal virtual MyWebApplicationFactory<TEntryPoint> WithWebHostBuilderCore(Action<IWebHostBuilder> configuration)
    {
    ...
    public void UseKestrel()
    {
        if (ServerStarted)
        {
            throw new InvalidOperationException();//TODO: Resources.UseKestrelCanBeCalledBeforeInitialization);
        }
    ...
    public void StartServer()
    {
        ...
        if (builder is null)
        {
            ...
            throw new InvalidOperationException();/*TODO Resources.FormatMissingBuilderMethod(
                nameof(IHostBuilder),
    ...
    private static void EnsureDepsFile()
    {
        ...
        {
            throw new InvalidOperationException();//TODO: Resources.FormatInvalidAssemblyEntryPoint(typeof(TEntryPoint).Name));
        }
        ...
        if (!depsFile.Exists)
        {
            throw new InvalidOperationException();/* TODO: Resources.FormatMissingDepsFile(
                depsFile.FullName,
    ...
    public HttpClient CreateDefaultClient(params DelegatingHandler[] handlers)
    {
        ...
        if (handlers == null || handlers.Length == 0)
        {
            ...
            else
            {
                if (_server is null)
                {
                    throw new InvalidOperationException();//TODO: Resources.ServerNotInitialized);
                }
    ...
    /// <summary>
    /// Configures <see cref="HttpClient"/> instances created by this <see cref="MyWebApplicationFactory{TEntryPoint}"/>.
    /// </summary>
    ...
    private sealed class DelegatedWebApplicationFactory : MyWebApplicationFactory<TEntryPoint>
    {
        ...
        internal override MyWebApplicationFactory<TEntryPoint> WithWebHostBuilderCore(Action<IWebHostBuilder> configuration)
        {
